...
class DailyProductSalesInventoryReport(models.TransientModel):
    ...
    def _compute_report_lines(self):
        ...
        # Sort by POS category sequence, then category name, then SKU
        cat_info_by_pid = {}
        for prod in self.env["product.product"].browse(list(result.keys())):
        ...
        for pid, data in result.items():
            ...
            data["pos_categ_seq"] = info["seq"]
            data["pos_categ_name"] = info["name"] or "(No Category)"
            data["pos_categ_id"] = info["id"]

        result_list = sorted(result.values(), key=lambda d: (
            d["pos_categ_seq"], d["pos_categ_name"], d.get("sku", ""), d.get("name", "")
        ))
        prev_categ_id = None
        for d in result_list:
            d["show_categ_header"] = (d["pos_categ_id"] != prev_categ_id)
            prev_categ_id = d["pos_categ_id"]
        return result_list
    ...
    def generate_xls_report(self):
        ...
        categ_format = workbook.add_format({"bold": True, "bg_color": "#D9E1F2", "border": 1})
        ...
        for line in report_lines:
            if line.get("show_categ_header"):
                sheet.merge_range(row, 0, row, 14, line.get("pos_categ_name", ""), categ_format)
                row += 1
