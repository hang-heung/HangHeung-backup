...
class ProductSaleAllShopReportWizard(models.TransientModel):
    ...
    def generate_report(self):
        ...
        for lv in line_vals:
            pid = lv['product_id'][0]
            shop_id = order_to_shop.get(
                lv['order_id'][0] if isinstance(lv['order_id'], (list, tuple)) else lv['order_id']
            )
            if not shop_id:
                continue
        ...
        # Column layout (DATA_START = 2, no POS category column – grouping rows used instead):
        #   0 (A): 產品編號
        #   1 (B): 產品名稱
        #   2+   : per-shop [qty, gross, net] x n shops
        #   last3: 總銷售數量, 總銷售總額, 總銷售淨額
        DATA_START = 2
        n_shops = len(shops)
        total_data_cols = n_shops * 3 + 3  # shop cols + 3 grand-total cols
        last_col = DATA_START + total_data_cols - 1
        ...
        fmt = {
            ...
            'categ':       workbook.add_format({'bold': True, 'bg_color': '#D9E1F2', 'border': 1, 'align': 'left'}),
        }
        ...
        # Row 0: company name
        worksheet.merge_range(0, 0, 0, last_col, company_name, fmt['title'])
        # Row 1: report title
        worksheet.merge_range(1, 0, 1, last_col, "產品銷售明細報告 (全線)", fmt['shop_header'])
        # Row 2: date range
        worksheet.write(2, 0, "日期區間:", fmt['title'])
        worksheet.write(2, 1, date_label)

        # Row 4: shop name headers
        row = 4
        worksheet.merge_range(row, 0, row, DATA_START - 1, '', fmt['shop_header'])
        col = DATA_START
        for shop in shops:
            worksheet.merge_range(row, col, row, col + 2, shop.name, fmt['shop_header'])
            col += 3
        worksheet.merge_range(row, col, row, col + 2, "合計", fmt['shop_header'])
        ...
        for product in products:
            ...
            categ_id   = cats[0].id   if cats else 0
            categ_name = cats[0].name if cats else '(No Category)'

            if categ_id != current_categ_id:
                worksheet.merge_range(row, 0, row, last_col, categ_name, fmt['categ'])
